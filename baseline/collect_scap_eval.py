...
def collect(base_dir: str, out_csv: str) -> pd.DataFrame:
    ...
    for root, _, files in os.walk(base_dir):
        for file in files:
            ...
            if str(meta.get("method", "")).lower().strip() not in {"scap", ""}:
                continue
            ...
            rows.append(
                {
                    "file_path": path,
                    "repo": repo,
                    "method": meta.get("method", "scap") or "scap",
...
def main():
    parser = argparse.ArgumentParser(description="Collect SCAP baseline evaluation into CSV.")
    parser.add_argument(
        "--lang",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Base directory to search; supports {lang}. Default: baseline/scap_{lang}",
    )
    parser.add_argument(
        "--out_csv",
        type=str,
        default=None,
        help="Output CSV path; supports {lang}. Default: baseline/evaluation_results_scap_{lang}.csv",
    )
    ...
    def resolve_out_csv(lang: str) -> str:
        if args.out_csv is None:
            return os.path.join(this_dir, f"evaluation_results_scap_{lang}.csv")
        ...
        if len(langs) > 1:
            ...
            return os.path.join(out, f"evaluation_results_scap_{lang}.csv")
        return out
