...
_DEFAULT_PATTERN = r"^evaluation_results_scap_(go|java|js|php|python)_info\.csv$"
...
def _infer_lang(path: str) -> str:
    ...
    m = re.match(r"evaluation_results_scap_([a-z]+)_info\.csv$", base)
...
def aggregate(inputs: list[str], out_repo_csv: str, out_overall_csv: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for col in group_cols:
        if col not in all_df.columns:
            raise SystemExit(f"Missing required column {col!r} in inputs; regenerate with baseline/collect_scap_info_eval.py")
...
def main():
    parser = argparse.ArgumentParser(description="Aggregate SCAP Com-Info evaluation across per-language CSVs.")
    parser.add_argument(
        "--baseline_dir",
        type=str,
        default=None,
        help="Directory containing evaluation_results_scap_{lang}_info.csv (default: this script's directory).",
    )
    ...
    if not inputs:
        raise SystemExit(f"No input SCAP-info CSVs found in {baseline_dir!r}")

    out_repo_csv = os.path.abspath(args.out_repo_csv or os.path.join(baseline_dir, "aggregate_scap_info_repo.csv"))
    out_overall_csv = os.path.abspath(args.out_overall_csv or os.path.join(baseline_dir, "aggregate_scap_info_overall.csv"))
